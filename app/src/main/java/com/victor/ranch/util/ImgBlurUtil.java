...
public class ImgBlurUtil {
    private static final String TAG = "ImgBlurUtil";
    private static final float SCALE = 1 / 8.0F;//default scale
    private static volatile ImgBlurUtil singleton = null;
    private Bitmap mBitmap;
    private int mRadius = 0;
    private float mScale = SCALE;
    private Context mContext;
    private BlurPolicy mPolicy = BlurPolicy.FAST_BLUR;//默认使用rs 模糊图片
     ...
     * @return
     */
    public static ImgBlurUtil getInstanse(){
        if (singleton == null) {
            synchronized (ImgBlurUtil.class) {
                if (singleton == null) {
                    singleton = new ImgBlurUtil();
                }
            }
        }
        return singleton;
     ...
     * @return
     */
    public Bitmap blurImg(Bitmap bitmap,int radius) {
        if(bitmap == null){
        ...
        if(SDK_INT > 8){// support library running on 2.3(API 9)  and higher 算法
            if(mPolicy == BlurPolicy.FAST_BLUR){
                ...
                return rsBlur(mContext,bitmap,radius,mScale);
            }
        } else{
            Log.d(TAG,"blur fast algorithm");
            return  fastBlur(bitmap,mScale,radius);
     ...
     * @return
     */
    public ImgBlurUtil policy(BlurPolicy policy){
        this.mPolicy = policy;
        return this;
    }
     ...
     * @return
     */
    public ImgBlurUtil scale(int scale){
     ...
     * @return
     */
    public ImgBlurUtil radius(int radius){
        this.mRadius = radius;
        return this;
    }
